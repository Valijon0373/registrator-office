  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        {[
          { label: 'Jami', value: appeals.length, color: 'text-slate-900' },
          { label: 'Yangi', value: appeals.filter((a) => a.status === 'yangi').length, color: 'text-blue-600' },
          { label: 'Jarayonda', value: appeals.filter((a) => a.status === 'jarayonda').length, color: 'text-amber-600' },
          { label: 'Kechikkan', value: appeals.filter((a) => a.status === 'kechikkan').length, color: 'text-rose-600' },
        ].map((item) => (
          <div
            key={item.label}
      ...
      <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/60">
        <div className="flex flex-col gap-4 border-b border-slate-100 p-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              type="button"
              className="flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600"
            >
        ...
        <div className="divide-y divide-slate-100">
          {appeals.map((appeal) => (
            <div
              key={appeal.id}
              className="flex flex-col gap-3 p-6 transition-colors hover:bg-slate-50/80 sm:flex-row sm:items-center sm:justify-between"
            >
              <div className="flex items-start gap-4">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-urspi-100 text-sm font-bold text-urspi-700">
                  {appeal.student.charAt(0)}
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-mono text-xs font-medium text-urspi-600">
                      {appeal.id}
                    </span>
                    <StatusBadge status={appeal.status} />
                  </div>
                  <p className="mt-1 font-medium text-slate-900">{appeal.student}</p>
