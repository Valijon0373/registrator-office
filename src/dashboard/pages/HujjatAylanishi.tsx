...
export default function HujjatAylanishi() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Hujjat aylanishi</h1>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/50">
                    ...
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-500">Hujjat</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-500">Kimdan</th>
                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase text-slate-500">Kimga</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {recentDocs.map((doc) => (
                    <tr key={doc.id} className="hover:bg-slate-50/80">
                      <td className="px-6 py-4 font-mono text-sm text-urspi-600">{doc.id}</td>
                      <td className="px-6 py-4 text-sm text-slate-900">{doc.name}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{doc.from}</td>
                      <td className="px-6 py-4 text-sm text-slate-600">{doc.to}</td>
              ...
              {departments.map((dept) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-urspi-500" />
